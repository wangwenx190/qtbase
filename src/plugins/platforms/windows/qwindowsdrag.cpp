...
static HRESULT startDoDragDrop(LPDATAOBJECT pDataObj, LPDROPSOURCE pDropSource, DWORD dwOKEffects, LPDWORD pdwEffect)
{
    ...
    for (;;) {
        ...
        if (::GetMessage(&msg, hwnd, 0, 0) > 0) {
            ...
            if (msg.message == WM_POINTERUPDATE && QWindowsApi::supportsPointerApi()) {

                const quint32 pointerId = GET_POINTERID_WPARAM(msg.wParam);

                POINTER_INFO pointerInfo{};
                static const auto pGetPointerInfo =
                    reinterpret_cast<decltype(&::GetPointerInfo)>(
                        QApiCache::instance().get(QApiCache::SD_User32, "GetPointerInfo"_L1));
                if (!pGetPointerInfo(pointerId, &pointerInfo))
                    return E_FAIL;
                ...
                if (msg.message == WM_POINTERLEAVE && QWindowsApi::supportsPointerApi())
                    return E_FAIL;
            }
        } else {
            return E_FAIL;
