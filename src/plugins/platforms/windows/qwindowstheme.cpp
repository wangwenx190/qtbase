...
[[maybe_unused]] [[nodiscard]] static inline QColor qt_accentColor(AccentColorLevel level)
{
    ...
    if (!colorSet) {
        qWarning() << "Windows QPA: qt_accentColor() cannot retrieve theme color.";
        return {};
    }
    switch (level) {
...
void QWindowsTheme::refreshFonts()
{
    ...
    if (pSystemParametersInfoForDpi)
        pSystemParametersInfoForDpi(SPI_GETICONTITLELOGFONT, sizeof(lfIconTitleFont), &lfIconTitleFont, 0, dpi);
    else
