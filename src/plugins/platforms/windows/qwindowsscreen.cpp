...
static float getMonitorSDRWhiteLevel(DISPLAYCONFIG_PATH_TARGET_INFO *targetInfo)
{
    ...
    static const auto pDisplayConfigGetDeviceInfo =
        reinterpret_cast<decltype(&::DisplayConfigGetDeviceInfo)>(
            QApiCache::instance().get(QApiCache::SD_User32, "DisplayConfigGetDeviceInfo"_L1));
    if (!pDisplayConfigGetDeviceInfo || !targetInfo)
                                       ...
                                       const std::vector<DISPLAYCONFIG_PATH_INFO> &pathGroup)
{
    static const auto pDisplayConfigGetDeviceInfo =
        reinterpret_cast<decltype(&::DisplayConfigGetDeviceInfo)>(
            QApiCache::instance().get(QApiCache::SD_User32, "DisplayConfigGetDeviceInfo"_L1));
    if (pathGroup.empty() || !pDisplayConfigGetDeviceInfo) {
        return;
    }
...
{
    static const auto pSetDisplayAutoRotationPreferences =
        reinterpret_cast<decltype(&::SetDisplayAutoRotationPreferences)>(
            QApiCache::instance().get(QApiCache::SD_User32, "SetDisplayAutoRotationPreferences"_L1));
    ...
    return pSetDisplayAutoRotationPreferences(orientationPreference);
}

Qt::ScreenOrientation QWindowsScreen::orientationPreference()
{
    static const auto pGetDisplayAutoRotationPreferences =
        reinterpret_cast<decltype(&::GetDisplayAutoRotationPreferences)>(
            QApiCache::instance().get(QApiCache::SD_User32, "GetDisplayAutoRotationPreferences"_L1));
    if (!pGetDisplayAutoRotationPreferences)
        return Qt::PrimaryOrientation;
    ORIENTATION_PREFERENCE orientationPreference = ORIENTATION_PREFERENCE_NONE;
    if (pGetDisplayAutoRotationPreferences(&orientationPreference)) {
