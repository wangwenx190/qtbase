...
{
    static const auto pRoGetActivationFactory =
        reinterpret_cast<decltype(&::RoGetActivationFactory)>(
            QApiCache::instance().get(QApiCache::SD_ComBase, "RoGetActivationFactory"_L1));
    static const auto pWindowsCreateStringReference =
        reinterpret_cast<decltype(&::WindowsCreateStringReference)>(
            QApiCache::instance().get(QApiCache::SD_ComBase, "WindowsCreateStringReference"_L1));
    if (!pRoGetActivationFactory || !pWindowsCreateStringReference) {
        return false;
    }
    ...
    if (FAILED(pWindowsCreateStringReference(uiViewSettingsId, uiViewSettingsIdLen, &uiViewSettingsIdRefHeader, &uiViewSettingsIdHs)))
        return false;
    ...
    HRESULT hr = pRoGetActivationFactory(uiViewSettingsIdHs, uiViewSettingsInteropRefId, reinterpret_cast<void **>(&uiViewSettingsInterop));
    if (FAILED(hr))
        return false;
